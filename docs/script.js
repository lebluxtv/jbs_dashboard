(function() {
    ...
    function getStoredPwd() {
        try {
            return localStorage.getItem(SB_PWD_KEY) || "";
        } catch {
            return "";
        }
    }
    ...
    function setStoredPwd(v) {
        try {
            localStorage.setItem(SB_PWD_KEY, v || "");
        } catch {}
    }
    ...
    function setTimerText(txt) {
        $$("#guess-timer, #gtg-timer").forEach(el => {
            el.textContent = txt;
        });
    }
    ...
    function setRoundNote(running) {
        ...
        if (!targets.length) {
            ...
            const candidates = Array.from(scope.querySelectorAll("small, .muted, .hint, span, div")).filter(el => el && typeof el.textContent === "string");
    ...
    function syncEventsStatusUI() {
        ...
        if (bQV) {
            bQV.textContent = String(qvUnreadEvents);
            bQV.style.display = qvUnreadEvents > 0 ? "" : "none";
        }
    ...
    function showTab(name) {
        ...
        try {
            localStorage.setItem("jbs.activeTab", name);
        } catch {}
    }
    ...
    (function initTab() {
        ...
        try {
            initial = localStorage.getItem("jbs.activeTab") || "overview";
        } catch {}
        showTab(initial);
    })();
    ...
    function setGuessMessage(t) {
        if (guessMsgEl) guessMsgEl.textContent = t || "";
    }
    ...
    function renderExcludeChips() {
        ...
        for (const id of GTG_EXCLUDED) {
            ...
            chip.addEventListener("click", () => {
                ...
                renderExcludeChips();
                saveLastSetup({
                    excludeGenreIds: Array.from(GTG_EXCLUDED)
                });
                requestPoolCount();
            });
    ...
    function saveLastSetup(setup) {
        ...
        try {
            localStorage.setItem(LAST_SETUP_KEY, JSON.stringify(merged));
        } catch {}
    }

    function loadLastSetup() {
        try {
            return JSON.parse(localStorage.getItem(LAST_SETUP_KEY) || "{}") || {};
        } catch {
            return {};
        }
    }
    ...
    function getCurrentSetupFromUI() {
        const {
            clean
        } = validateFilters(collectFilters());
        return {
    ...
    function saveLastSetupFromUI() {
        try {
            saveLastSetup(getCurrentSetupFromUI());
        } catch {}
    }
    ...
    function validateFilters(raw) {
        ...
        if (targetScore != null && !isNum(targetScore)) {
            errs.push("Score cible invalide.");
            targetScore = null;
        }
        if (isNum(targetScore)) {
            targetScore = Math.max(1, Math.min(999, Math.trunc(targetScore)));
        }
        return {
            ok: errs.length === 0,
            errs,
    ...
    function sendRawDoActionById(actionId, argsObj) {
        try {
            ...
            const wireArgs = Object.assign({}, argsObj || {}, {
                _json: JSON.stringify(argsObj || {})
            });
            const payload = {
                request: "DoAction",
                id: "DoAction",
    ...
    async function safeDoAction(actionName, args) {
        try {
            if (!sbClient) {
                appendLog("#guess-log", "Client Streamer.bot non initialisé.");
                return;
            }
            const wire = Object.assign({}, args || {}, {
                _json: JSON.stringify(args || {})
            });
    ...
    function setGuessHandlers() {
        ...
        seriesCancelBtn?.addEventListener("click", () => {
            ...
            safeDoAction("GTG Series Cancel", {
                reason: "manual"
            });
        });
    ...
    function reconnectSB() {
        try {
            if (window.sbClient && sbClient && typeof sbClient.disconnect === "function") sbClient.disconnect();
        } catch {}
        connectSB();
    }
            ...
            if (data && data.widget === "gtg") {
                if (data.type === "bootstrap") {
                    ...
                    if (guessYearFromInput) {
                        guessYearFromInput.min = String(OL);
                        guessYearFromInput.max = String(NW);
                    }
                    if (guessYearToInput) {
                        guessYearToInput.min = String(OL);
                        guessYearToInput.max = String(NW);
                    }
                ...
                if (data.type === "start") {
                    ...
                    setRunning(true);
                    const endMs = Number.isFinite(data.endsAtUtcMs) ? Number(data.endsAtUtcMs) : Number.isFinite(data.endTs) ? Number(data.endTs) : Number.isFinite(data.endsAt) ? Number(data.endsAt) : NaN;
                    if (Number.isFinite(endMs)) startRoundTimer(endMs);
                    const targetName = extractTargetNameFromPayload(data);
                    if (targetName) appendLogDebug("target", {
                        name: targetName
                    });
                ...
                if (data.type === "tick") {
                    const endMs = Number.isFinite(data.endsAtUtcMs) ? Number(data.endsAtUtcMs) : Number.isFinite(data.endTs) ? Number(data.endTs) : Number.isFinite(data.endsAt) ? Number(data.endsAt) : NaN;
                    if (Number.isFinite(endMs)) startRoundTimer(endMs);
                    appendLogDebug("tick.payload", {
                        endsAtUtcMs: data.endsAtUtcMs ?? data.endTs ?? data.endsAt
                    });
                    return;
                }
                if (data.type === "reveal") {
