(function () {
  ...
  async function gtgEnd(){
    if (!client) return setGuessMessage('Client SB indisponible.');
    try{ await client.doAction({ name: "GTG End" }); }catch{}
    DashboardStatus.guess.setStatus(false);
    DashboardStatus.setStatus('guess', false);
  ...
  (function checkResetPwd(){ if (getQS('resetpwd') === '1'){ clearStoredPwd(); history.replaceState(null, '', location.pathname); alert('Mot de passe Streamer.bot effacé localement. Rechargez et saisissez-le à nouveau.'); } })();
  ...
  async function setTtsAutoReader(enabled){
    try { await client.doAction({ name: "TTS Auto Message Reader Switch ON OFF", args: { mode: enabled ? "on" : "off" } });
